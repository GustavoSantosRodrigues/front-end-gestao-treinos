...
export function AddExerciseModal({
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      type ExerciseWithId = GetWorkoutDay200ExercisesItem & {
        exerciseId?: string;
      };

      // mantém todos os existentes + adiciona o novo no final
      const updatedExercises = [
        ...allExercises.map((ex) => ({
          ...
          exerciseId: (ex as ExerciseWithId).exerciseId,
        })),
        {
          order: allExercises.length + 1,
          name: selected.name,
          sets: 4,
          reps: 10,
      ...
      await clientFetch(`/workout-plans/${workoutPlanId}`, {
        ...
        body: JSON.stringify({
          workoutDays: [{ weekDay, exercises: updatedExercises }],
        }),
      });
      ...
      <div className="flex max-h-[85vh] w-full flex-col rounded-t-2xl bg-background">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-heading text-xs text-muted-foreground">
              {allExercises.length} exercícios no dia
            </span>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {MUSCLE_GROUPS.map((m) => (
              <button
                ...
                className={`shrink-0 rounded-full px-3 py-1.5 font-heading text-xs font-semibold transition-colors ${
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {loading ? (
            ...
            <p className="py-12 text-center font-heading text-sm text-muted-foreground">
