...
import { CircleHelp, Zap, RotateCcw, Check, EyeOff, Play, ArrowLeftRight } from "lucide-react";
...
export function ExerciseCard({ exercise, workoutPlanId, workoutDayId, allExercises, sessionId, isTrainer, weekDay }: ExerciseCardProps) {
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-border p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {isTrainer && (
              <Button variant="ghost" size="icon" onClick={() => setShowSwap(true)}>
                <ArrowLeftRight className="size-4 text-muted-foreground" />
              </Button>
            )}

            {exercise.gifUrl && (
              <Button variant="ghost" size="icon" onClick={() => setShowGif((prev) => !prev)}>
                {showGif ? (
        ...
        {sessionId && (
          <>
            ...
            {sets.map((set, index) => (
              <div
                key={index}
                className={`grid grid-cols-[40px_1fr_1fr_40px] items-center gap-2 rounded-lg p-1 ${set.completed ? "bg-primary/5" : ""}`}
              >
                ...
                <button
                  ...
                  className={`flex size-9 items-center justify-center rounded-full border-2 transition-all duration-300 ${
                    set.completed ? "border-primary bg-primary text-primary-foreground scale-110"
                    : set.saving ? "border-primary/50 bg-primary/10"
                ...
                >
                  {set.saving ? (
                    ...
                    <span className="font-heading text-xs font-semibold text-muted-foreground">{set.setNumber}</span>
                  )}
                </button>
              </div>
            ))}
